...
*/
template <typename AgentType, typename WeightType = double> class Network {
public:
  ...
  Network(std::vector<std::vector<size_t>> &&neighbour_list,
  ...
  */
  ...
  n_edges(std::optional<std::size_t> agent_idx = std::nullopt) const {
  ...
  */
  ...
  [[nodiscard]] std::span<WeightT> get_weights(std::size_t agent_idx) {
    return std::span<WeightT>(weight_list[agent_idx].data(),
                              weight_list[agent_idx].size());
  }
  ...
  void set_weights(std::size_t agent_idx,
    ...
    if (neighbour_list[agent_idx].size() != weights.size()) {
      throw std::runtime_error(
          "Network::set_weights: tried to set weights of the wrong size!");
    }
  ...
  */
  void set_neighbours_and_weights(std::size_t agent_idx,
    ...
    std::fill(weight_list[agent_idx].begin(), weight_list[agent_idx].end(),
              weight);
  }
  ...
  */
  void set_neighbours_and_weights(std::size_t agent_idx,
    ...
    if (buffer_neighbours.size() != buffer_weights.size()) {
      throw std::runtime_error("Network::set_neighbours_and_weights: both "
                               "buffers need to have the same length!");
    }
  ...
  */
  void toggle_incoming_outgoing() {
    std::vector<std::vector<size_t>> neighbour_list_transpose(
        n_agents(), std::vector<size_t>(0));
    std::vector<std::vector<WeightT>> weight_list_transpose(
        n_agents(), std::vector<WeightT>(0));

    for (size_t i_agent = 0; i_agent < n_agents(); i_agent++) {
      for (size_t i_neighbour = 0; i_neighbour < neighbour_list[i_agent].size();
           i_neighbour++) {
