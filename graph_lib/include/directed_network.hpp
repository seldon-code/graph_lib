...
public:
  ...
  /*
  Gives the number of edges going out/coming in at agent_idx, depending on the
  value of direction(). If agent_idx is nullopt, gives the total number of edges
  */
  [[nodiscard]] std::size_t
  n_edges(std::optional<std::size_t> agent_idx = std::nullopt) const override {
    if (agent_idx.has_value()) {
      return this->neighbour_list[agent_idx.value()].size();
    } else {
      return std::transform_reduce(
          this->neighbour_list.cbegin(), this->neighbour_list.cend(), 0,
          std::plus{},
  ...
  /*
  Returns the current directionality of the adjacency list. That is if incoming
  or outgoing edges are stored.
  */
  [[nodiscard]] const EdgeDirection &direction() const { return _direction; }

  /*
  Set the edge weights going out/coming in at agent_idx
  */
  void set_weights(std::size_t agent_idx,
                   const std::span<const WeightT> weights) {
    if (this->neighbour_list[agent_idx].size() != weights.size()) {
      throw std::runtime_error("DirectedNetwork::set_weights: tried to set "
                               "weights of the wrong size!");
    }
  ...
  */
  void set_edge(std::size_t agent_idx, std::size_t index_neighbour,
                std::size_t agent_jdx) {
    this->neighbour_list[agent_idx][index_neighbour] = agent_jdx;
  }

  /*
  Sets the neighbour indices and sets the weight to a constant value at
  agent_idx
  */
  void set_neighbours_and_weights(std::size_t agent_idx,
                                  std::span<const size_t> buffer_neighbours,
                                  const WeightT &weight) {
    this->neighbour_list[agent_idx].assign(buffer_neighbours.begin(),
                                           buffer_neighbours.end());
    this->weight_list[agent_idx].resize(buffer_neighbours.size());
    std::fill(this->weight_list[agent_idx].begin(),
              this->weight_list[agent_idx].end(), weight);
  }
  ...
  */
  void set_neighbours_and_weights(std::size_t agent_idx,
                                  std::span<const size_t> buffer_neighbours,
                                  std::span<const WeightT> buffer_weights) {
    if (buffer_neighbours.size() != buffer_weights.size()) {
      throw std::runtime_error("DirectedNetwork::set_neighbours_and_weights: "
                               "both buffers need to have the same length!");
    }

    this->neighbour_list[agent_idx].assign(buffer_neighbours.begin(),
                                           buffer_neighbours.end());
    this->weight_list[agent_idx].assign(buffer_weights.begin(),
                                        buffer_weights.end());
  }
  ...
  */
  void set_edge_weight(std::size_t agent_idx, std::size_t index_neighbour,
                       WeightT weight) override {
  ...
  */
  void push_back_neighbour_and_weight(size_t agent_idx_i, size_t agent_idx_j,
                                      WeightT w) override {
  ...
  */
  void toggle_incoming_outgoing() {
    std::vector<std::vector<size_t>> neighbour_list_transpose(
        this->n_agents(), std::vector<size_t>(0));
    std::vector<std::vector<WeightT>> weight_list_transpose(
        this->n_agents(), std::vector<WeightT>(0));

    for (size_t i_agent = 0; i_agent < this->n_agents(); i_agent++) {
      for (size_t i_neighbour = 0;
           i_neighbour < this->neighbour_list[i_agent].size(); i_neighbour++) {
  ...
  simultaneously, changes its representation. Example: N(inc) -> N^T(out)
  */
